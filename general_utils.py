...
def split_with_span(s):
    if s.split() == []:
        ...
        return zip(*[(m.group(0), (m.start(), m.end()-1)) for m in re.finditer(r'\S+', s)])#\S匹配非空白字符

def free_text_to_span(free_text, full_text):#(gold_answer, span_answer)
...
def get_context_span(context, context_token):#str,list[str]
    p_str = 0
    p_token = 0
    t_span = []
    while p_str < len(context):
        if re.match('\s', context[p_str]):#跳过了所有的空白，今天下午在这个地方耽误了一下午，看了一些re.match和正则表达式的东西，晚上又干起来主席的活。
            p_str += 1
            continue

        token = context_token[p_token]#上面跳过了所有的空白，直到遇到一个真正的单词，这个时候就是p_token在发挥作用了
        token_len = len(token)
        if context[p_str:p_str + token_len] != token:
            log.info("Something wrong with get_context_span()")#怪不得这个地方的log要标红，因为没有log这个变量的定义
            return []#something wrong这个词儿用的好，估计作者自己都不知道是什么问题
        t_span.append((p_str, p_str + token_len))

        p_str += token_len
        p_token += 1
    return t_span #各个context_token这个list中的token对应于context这个字符串中的，单词首尾的下标 [[],[],[]]一句话作为list，这句话里面的每个单词再是一个二元素的list

def find_answer_span(context_span, answer_start, answer_end):#[[],[],[]]一句话作为list，这句话里面的每个单词再是一个二元素的list，二元=[start,end]都是char base的
    if answer_start == -1 and answer_end == -1:
        return (-1, -1)

    t_start, t_end = 0, 0
    for token_id, (s, t) in enumerate(context_span):
        if s <= answer_start: #s要不断的逼近answer_start ,s最起码要，<= answer_start
            t_start = token_id
        if t <= answer_end:
            t_end = token_id
